...
namespace WebSocketManager.Client
{
    public class Connection
    {
        ...
        public async Task StartConnectionAsync(string uri)
        {
            ...
            await Receive(_clientWebSocket, async (receivedMessage) =>
            {
                ...
                else if (receivedMessage.MessageType == MessageType.MethodInvocation)
                {
                    ...
                    try
                    {
                        invocationDescriptor = TypeSerializer.DeserializeFromString<InvocationDescriptor>(receivedMessage.Data);
                    ...
                    else
                    {
                        ...
                        var message = new Message()
                        {
                            ...
                            Data = invokeResult.SerializeAndFormat()
                        };
                        var bytes = Encoding.UTF8.GetBytes((message.SerializeAndFormat()));
                ...
                {
                    var invocationResult = TypeSerializer.DeserializeFromString<InvocationResult>(receivedMessage.Data);
        ...
        public async Task<T> SendAsync<T>(InvocationDescriptor invocationDescriptor)
        {
            ...
            var bytes = Encoding.UTF8.GetBytes((new Message { MessageType = MessageType.MethodInvocation, Data = TypeSerializer.SerializeToString(invocationDescriptor) }).SerializeAndFormat());
        ...
        {
            await _clientWebSocket.CloseOutputAsync(WebSocketCloseStatus.NormalClosure, "Graceful shutdown. Bye!", CancellationToken.None).ConfigureAwait(false);
        }
        ...
        private async Task Receive(ClientWebSocket clientWebSocket, Action<Message> handleMessage)
        {
            ...
            {
                ArraySegment<Byte> buffer = new ArraySegment<byte>(new Byte[1024 * 4]);///here comes the pipereader..
                ...
                {
                    var message = TypeSerializer.DeserializeFromString<Message>(serializedMessage);
